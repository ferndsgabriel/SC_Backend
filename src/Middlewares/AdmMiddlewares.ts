...
export default async function AdmMiddlewares(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        const { sub } = verify(token, process.env.AJWT_SECRET as string) as Payload; // tento pegar o sub dentro do token
